...
ADMIN_ID = "5399171337"  # string sifatida
# ───────────────────────────────────────────
...
def show_test_participants(message):
    ...
    results.sort(key=lambda x: -x[1])  # eng yuqori natijadan pastga
...
def handle_all_messages(message):
    ...
    if uid == ADMIN_ID and text.startswith('*') and text.count('*') >= 2:
        try:
            ...
            tests[new_id] = {
                "correct": correct_str,          # string sifatida saqlaymiz
                "total": total,
